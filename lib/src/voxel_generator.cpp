...
std::size_t VoxelGenerator::pointsToVoxels(const sensor_msgs::msg::PointCloud2 & input_pointcloud_msg,
  ...
  std::vector<float> & num_points_per_voxel)
{
  ...
  // for (sensor_msgs::PointCloud2ConstIterator<float> x_iter(input_pointcloud_msg, "x"), y_iter(input_pointcloud_msg, "y"),
  //        z_iter(input_pointcloud_msg, "z"),inten_iter(input_pointcloud_msg,"intensity");
  //        x_iter != x_iter.end(); ++x_iter, ++y_iter, ++z_iter,++inten_iter) {
  ...
  for(size_t i=0;i < pcl_pc_ptr->size(); i++){
     ...
     pcl::PointXYZ pc_point = pcl_pc_ptr->at(i);
      point[0] = pc_point.x;
      point[1] = pc_point.y;
      point[2] = pc_point.z;
      point[3] = 1.0;
